...
class BLOSUMLoss(nn.Module):
    ...
    def forward(self, logits, labels):
        ...
        """
        # ensure that the model does not predict 0,1,2 (= <unk>, <pad>, <mask>)
        logits[:, :, 0:3] = -float('inf')

        # ignore the labels with index -100
        predicted_logits = logits[labels!=-100]
        true_labels = labels[labels!=-100]

        predicted_indices = predicted_logits.argmax(dim=-1)  # dim (batch_size, seq_length)

        blosum_scores = self.blosum_matrix[true_labels.view(-1)-3, predicted_indices.view(-1)-3] #-3 since <unk>, <mask> and <pad> are not in blosum
